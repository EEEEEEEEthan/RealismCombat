...
## 存档正文第一个字符串（全 ASCII，UTF-8 字节数等于 length）；读写头时内部校验
...
## 从文件当前位置读取快照头；失败（含格式不符）返回 null
static func read_header_from_file(file_access: FileAccess) -> SaveSnapshot:
	if not _read_leading_file_string(file_access):
		return null
...
static func preview_path(path: String) -> SaveSlotPreview:
	...
	if file_access == null:
		return preview
	var read_snapshot: SaveSnapshot = read_header_from_file(file_access)
	file_access.close()
...
static func _write_leading_file_string(file_access: FileAccess) -> void:
	file_access.store_buffer(_FILE_MAGIC.to_utf8_buffer())

static func _read_leading_file_string(file_access: FileAccess) -> bool:
