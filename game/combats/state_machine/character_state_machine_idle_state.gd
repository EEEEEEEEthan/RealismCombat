...
func player_turn_choose_from_body_part() -> ActionParameter:
	...
	dialogue.text = "%s的回合!" % machine.combat.bbcode_character_name(machine.battler)
	...
	menu.title = "%s的回合" % machine.combat.bbcode_character_name(machine.battler)
...
func player_turn_choose_action(from_body: BodyPart) -> ActionParameter:
	var menu = Dialogues.create_menu_dialogue()
	menu.title = "%s的回合>%s" % [
		machine.combat.bbcode_character_name(machine.battler),
		from_body.part_name(),
	]
	var options: Array[MenuItemData]
...
func player_turn_choose_target(action: Action, from_body: BodyPart) -> ActionParameter:
	...
	for target_battler in machine.combat.characters:
		if not action.static_valid_to_character(target_battler).success:
			continue
		var data = MenuItemData.new("%s.." % target_battler.character_name)
		var outcome = action.dynamic_valid_to_character(target_battler)
		if outcome.success:
			data.description = "选择%s为目标" % machine.combat.bbcode_character_name(target_battler)
		options.append(data)
		targets.append(target_battler)
	options.append(MenuItemData.new(&"返回", false, &"返回上一级"))
	menu.options = options
	while true:
		var choice = await menu.pressed
...
func player_turn_choose_target_body(
	...
	menu.title = "%s的回合>%s>%s>%s" % [
		...
		from_body.part_name(),
		action.get_name(),
		machine.combat.bbcode_character_name(to_battler),
	]
	var options: Array[MenuItemData]
	var body_list: Array[BodyPart]
	for to_body in to_battler.all_body_parts:
	...
	for action: Action in from.actions:
		for from_body: BodyPart in from.all_body_parts:
			if not action.valid_from_body(from_body):
				continue
			for to_battler: Battler in machine.combat.characters:
				if not action.valid_to_character(to_battler):
					continue
				for to_body: BodyPart in to_battler.all_body_parts:
