"""RealismCombat 的 egent 聊天 CLI。

运行前在项目根目录配置 ``.egent/.model.toml``：

    copy .egent/.model.toml.example .egent/.model.toml
    egent.bat --update
"""
...
from hot_reload import reload_agent
...
async def chat() -> None:
    ...
    async def assign(title: str, description: str) -> str:
        ...
        @param description: 需求文本；应描述需求背景和精准内容。上下文会保持，第二次 assign 起可直接描述修订方案。
        """
        nonlocal leader
        ...
        try:
            success, report = await workflow_instance.assign(description, title)
        except Exception as exception:  # pylint: disable=broad-exception-caught
            report = (
                f"开发工作流异常：{type(exception).__name__}: {exception}\n\n"
            ...
            if git_output:
                report += f"\n\n{git_output}"
            return report
        report = f"{report}\n\n开发日志见.egent/.temp/task-{workflow_instance.task_id}.log"
        if not success:
            git_output = discard_unstaged_changes()
            if git_output:
                report += f"\n\n{git_output}"
    ...
    def git_commit(commit_message: str) -> str:
        ...
        project_config_path = Path("pyproject.toml")
        ...
        if version_match:
            package_version = (
                ...
                f"{int(version_match.group(3)) + 1}"
            )
            project_config_path.write_text(
                project_config_text[:version_match.start()]
                + f'version = "{package_version}"'
                + project_config_text[version_match.end():],
                encoding="utf-8",
            )
        return_code, _ = shell_tools.run_command("git", "add", "-A")
        if return_code != 0:
            return "git add 失败"
        return_code, commit_output = shell_tools.run_command("git", "commit", "-m", commit_message)
        if return_code != 0:
            return f"git 提交失败：\n{commit_output or ''}"
        if package_version:
            return f"已提交 v{package_version}"
    ...
    leader = egent.agent.Agent(
        ...
        system_prompt=(
            ...
            "如果他让你修改项目，你需要提出方案。提出方案后和他核对，在他明确表达可以开始执行前不可开始执行。\n"
            "给出的方案应该措辞精炼，不说废话，以最简洁的方式给出方案。\n"
            f"执行修改请使用 {assign.__name__}，而不要亲自执行。为了防止你事必躬亲，我拿掉了你的编辑权限。\n"
        ),
        tools=(assign, egent.builtin_tools.test_tools.run_regression_test, git_commit),
    )
    workflow_instance = workflow.Workflow(leader)
    ...
    while True:
        ...
        percentage = leader.tokens / leader.auto_summarize_threshold * 100
        print(f"[tokens({leader.name}): {leader.tokens / 1000:.1f}k, {percentage:.0f}%]")
        reload_agent(leader)
