...
public class ChargeAttack(Character actor, BodyPart actorBodyPart, Combat combat)
	: AttackBase(actor, actorBodyPart, combat, 1, 2)
{
	...
	public override string? PreCastText => null;
	public override string CastText => $"{actor.name}用{actorBodyPart.Name}撞击{target.name}的{targetObject.Name}!";
}
