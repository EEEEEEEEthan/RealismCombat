...
public sealed class GameClient : IDisposable
{
	...
	public async Task<string> SendCommand(string command, int timeoutMs)
	{
		...
		try
		{
			lock (_sync)
			{
				if (_client is null || _stream is null || _reader is null || _writer is null || !_client.Connected)
				{
					return "未连接到游戏";
				}
			}
	...
	void StartGodotProcess(string godotPath, string projectRoot, int port)
	{
		...
		lock (_sync)
		{
			...
			try
			{
				_logWriter = new StreamWriter(new FileStream(logPath, FileMode.Create, FileAccess.Write, FileShare.Read), new UTF8Encoding(false)) { AutoFlush = true };
			}
			catch { }
