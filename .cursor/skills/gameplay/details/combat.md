...
- 每 tick 大循环开头：`Combat._try_finish_battle()` 判定阵营是否仍有存活 `Battler`。
- **我方全灭** → `GenericDialogue` 文案「战斗失败…」→ `queue_free()` 战斗节点 → `Game` 回主菜单。
- **敌方全灭**（先保证我方未全灭）→「战斗胜利！」→ 同上。`Game` 在菜单选「测试项」时用 `%Combat.create_instance()` 开战，战后 `combat = null` 并回到菜单。
