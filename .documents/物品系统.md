...
- `ItemData.weight` - 单个物品的重量（公斤）
- `ItemData.length` - 物品长度（米），如果是武器则为武器长度，否则为最长径
- `ItemData.GetTotalWeight()` - 获取该数量物品的总重量
...
- `weight` - 物品重量（公斤）
- `length` - 物品长度（米），如果是武器则为武器长度，否则为最长径
...
- `Arm` - 武器类型（用于手臂装备）
- `Knife`、`Sword`、`Hammer`、`Axe`、`Spear`、`Halberd` - 武器子类型
- `Shield` - 盾牌类型
...
- 所有枚举值都有中文文档注释说明其用途
- 例如：`EquipmentType.Arm | EquipmentType.Knife` 表示刀类武器

位于 `Scripts/Data/ItemData.cs` 的 `EquipmentType` 枚举

#### 装备类型名称扩展

为了在用户界面中显示装备类型，提供了 `EquipmentTypeExtensions` 扩展方法。

**扩展方法：**
- `GetName(this EquipmentType type)` - 获取完整的装备类型名称，例如"头盔内衬"、"胸甲内衬"、"单手武器或刀"
- `GetShortName(this EquipmentType type)` - 获取简短的装备类型名称，例如"内衬"、"中层"、"外套"，用于紧凑显示

**用途：**
- 在物品选择菜单的标题中显示槽位类型：`物品槽[{GetShortName()}]`
- 在物品选项的描述中显示物品类型：`[{GetShortName()}]`
- 支持多类型组合，使用"或"连接多个类型名称

位于 `Scripts/Extensions/EquipmentTypeExtensions.cs`
...
**空槽选择菜单（物品选择菜单）：**
- 标题显示为"物品槽[{槽位类型}]"，例如"物品槽[内衬]"，简洁显示当前槽位需要的装备类型
- 每个物品选项显示物品名称和数量（如"皮帽 x2"）
- 每个物品选项的description显示物品的装备类型信息，格式为"[{物品类型}]"，例如"[内衬]"、"[中层]"、"[单手武器或刀]"
- 可装备的物品正常显示且可选
- 不可装备的物品显示但禁用（灰色或不可点击），用户可以看到但不能选择
- 使用 `SlotData.CanPlace` 方法判断物品是否可以放入槽位
