...
- 游戏循环中展示二级菜单：
  ...
  - `装备`：当队伍只有一人时，跳过角色选择直接进入身体部位列表；从装备界面返回时直接回到章节菜单，避免额外的角色选择步骤
  - 第一章序章菜单中，“走吧...” 需先在装备界面穿好板甲与长剑才可选；未达成时选项置灰并提示“你需要先装备好盔甲和剑”
  - 序章遭遇战敌方名称为“贵族兵”（贵族士兵），用于保持 UI 排版不溢出

### 装备菜单流程

- 入口：脚本阶段 `_1_Equip` 中选择“装备”后调用 `ShowEquipmentFlow()`，用于管理角色身上的装备与物品栏
- 角色选择：无角色时提示“没有可用角色”；仅有一名角色则直接进入；多角色时使用 `MenuDialogue` 列出姓名并附带“返回”项，选“返回”会退出整个装备流程
- 身体部位选择：仅展示存在槽位的部位，标题使用 `BodyPart.GetNameWithEquipments()` 附带已穿装备摘要；菜单标题为 `{角色名}的装备`；描述在有装备时显示 `已装备:\n武装衣, 皮带` 等列表；菜单允许返回上一层，若角色不存在可装备部位会提示“没有可装备的部位”并返回
- 槽位展开：进入具体部位后只展示 `VisibleInMenu` 为真的槽位，空槽显示 `#空` 并在描述中列出可接受的标志名称，已占用槽显示装备名和 `FormatItemDescription()` 生成的类型+描述；若当前容器是嵌套装备，会额外出现“卸下”项，将该装备移回物品栏并提示成功
- 槽位标题：已占用槽位的标题会附带其子装备，例如腰带挂长剑显示 `#皮带[长剑]`
- 导航展示：导航链格式为 `{角色名}>{身体部位}>{槽位或装备}`，空槽使用槽位可接受的标志名称，嵌套装备继续追加装备名
- 空槽换装：选择空槽后会从物品栏筛选可匹配标志的装备生成菜单，选中后直接装备并从物品栏移除；不匹配时捕获异常并提示“装备类型不匹配，无法更换”；选择返回会退出当前槽位选择
- 嵌套容器：槽位已有装备时视为容器继续展开其内部槽位，遵循同样的可见性与卸下规则，便于编辑多层穿戴结构（如内衬→链甲→外套→腰带→武器）
- 退出规则：在任意菜单选择“返回”即退回上一层；单角色流程在完成一次部位编辑后返回主菜单，避免陷入无终止循环
...
- 支持以下指令：
  ...
  - `game_select_option`：模拟选项选择，优先调用 `MenuDialogue.SelectAndConfirm`，若当前为 `GenericDialogue` 则转而调用其 `SelectAndConfirm`，其他对话框会打印错误并触发 `McpCheckpoint`
