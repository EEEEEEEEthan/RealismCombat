...
**菜单堆叠宽度控制：**
- 菜单在HBoxContainer中横向堆叠显示
- 只有最后2个菜单保持完全可见（宽度16），其余菜单宽度为0（折叠）
- 宽度变化使用Lerp平滑过渡，提供流畅的视觉体验
- 当菜单数量少于2个时，所有菜单都保持完全可见
...
- **上下方向键**：在选项间移动选择
- **确认键**：确认当前选择的选项
- **ESC键**：如果对话框有默认返回选项（通过 `Initialize` 的 `onReturn` 参数提供），按下 ESC 键会触发返回选项
...
2. 使用 `Initialize(DialogueData, active, onReturn, returnDescription)` 初始化数据
   - `data` - 对话框数据
   - `active` - 是否激活（默认 true）
   - `onReturn` - 返回按钮回调（可选，如果提供则自动添加"返回"选项）
   - `returnDescription` - 返回按钮描述文本（可选）
3. 添加到 `dialogues` 容器
4. 设置为激活状态（自动输出MCP信息）

**返回按钮：**
- 如果 `Initialize` 时提供了 `onReturn` 参数，系统会自动在选项列表末尾添加"返回"选项
- 返回按钮的 `onConfirm` 会先调用 `QueueFree()` 删除对话框，然后执行 `onReturn()` 回调
- 返回选项可以通过上下方向键选择后确认，也可以直接按 ESC 键触发
- 这样可以简化多层菜单的实现，无需手动添加返回选项
...
- 新对话框（二级菜单）叠加在旧对话框（一级菜单）之上，旧对话框保留不删除
- 栈顶对话框（最新创建的对话框）始终为激活状态，可以接收用户输入
- 二级菜单销毁后，一级菜单自动恢复激活状态（通过 `_Process` 自动管理）
- 典型用例：物品栏作为二级菜单叠加在游戏菜单之上，返回时只关闭物品栏，游戏菜单保留
- 使用 `onReturn` 参数可以方便地为二级菜单添加返回功能，返回按钮会自动删除对话框
...
### PrinterLabelNode - 打字机效果标签组件
位于 `Scripts/Nodes/Components/PrinterLabelNode.cs`
...
**特性：**
- `interval` - 字符显示间隔时间（秒）
- `speedUp` - 是否加速播放（自动根据输入状态调整）
- `Printing` - 是否正在打印中
- 每次显示字符时自动播放音效（通过 `ProgramRootNode` 的音频系统）
