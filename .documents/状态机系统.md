...
- **状态层级**: `ProgramRoot` 拥有程序级状态（`MenuState`、`GameState`），`GameState` 拥有游戏级状态（`PrepareState`、`CombatState`），`CombatState` 拥有战斗级子状态（`TurnProgressState`、`ActionState`、`ReactionState`）
- **状态与节点的双向引用**: 状态持有并管理节点的生命周期，节点持有状态引用以便访问上下文

## 逐帧 Update 驱动

- 调用链：`ProgramRoot._Process(delta)` → `ProgramRoot.State.Update(delta)`
- 目标：由 `IStateOwner` 驱动所有状态的逐帧逻辑，使状态内部可按需复写 `Update(dt)`
- 涉及文件：
  - `Scripts/StateMachine/State.cs` - `State.Update(double dt)` 基类方法
  - `Scripts/Nodes/ProgramRoot.cs` - `_Process` 中调用 `State.Update`
...
- `Scripts/StateMachine/CombatStates/` - 战斗级子状态（TurnProgressState、ActionState、ReactionState）
...
### CombatState (战斗状态)
战斗进行中的状态，创建并管理 `Combat` 节点，实现 `IStateOwner` 以拥有战斗级子状态。

- `CombatState` 持有 `Combat` 节点引用，在构造时创建并挂载节点，退出时销毁
- `Combat` 节点持有 `CombatState` 引用，可访问状态上下文
- 实现 `IStateOwner` 接口，拥有战斗级子状态，默认初始化为 `TurnProgressState`
- 文件：`Scripts/StateMachine/GameStates/CombatState.cs`、`Scripts/Nodes/Combat.cs`

## 战斗级子状态

### TurnProgressState (回合进行状态)
回合进行中的状态，`CombatState` 的默认初始子状态。
