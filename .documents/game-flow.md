...
## 生命周期
- 新游戏：构造时创建 `Ethan`，将 `CottonLiner`、`CottonPants`、`Belt` 穿戴好，把 `LongSword` 放入物品栏，随后启动主循环
...
- 开场对白仅保留“拿上长剑”，已移除“穿上旧盔甲”的描述
- 创建 `CombatNode` 并实例化敌人“贵族兵”（默认携带长剑，行动点为最大值一半）
