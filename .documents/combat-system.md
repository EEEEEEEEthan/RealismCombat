...
- 当前实现的攻击类型：
  ...
  - `HeadbuttAttack`（Special 特殊）：只允许头使用；头槌攻击腿部时闪避影响 0.98、格挡影响 0.95，攻击手臂时闪避影响 0.95、格挡影响 0.92，攻击其他部位时闪避/格挡影响 0.35；若目标是手臂或躯干，有概率令目标获得[倒伏] Buff，概率为目标总重量/发起方总重量；若目标是腿部，发起方必定获得[倒伏] Buff；AI 只会用头槌攻击头部
...
- 反应类型包括：
  - 格挡（Block）：消耗 1 点反应，选择身体部位或装备承受伤害，伤害会转移到格挡目标；若格挡目标是当前行动使用的身体部位或其装备，会打断自身行动；格挡失败时，若身体部位受到的伤害 ≥ 该部位最大血量的 30%，会打断当前行动
  - 闪避（Dodge）：消耗 1 点反应，打断自身当前行动并躲开攻击，攻击完全落空
  - 承受（Endure）：不消耗反应，正常承受伤害；若身体部位受到的伤害 ≥ 该部位最大血量的 30%，会打断当前行动
- 闪避必定打断自身当前行动；格挡分两种情况：使用当前行动部位格挡会立即打断，格挡成功不会因伤害打断，格挡失败且受伤 ≥30% 血量时打断
- 伤害打断机制：
  - 格挡成功时不会触发伤害打断判定
  - 格挡失败或承受时，若身体部位受到的伤害 ≥ 该部位最大血量的 30%，会立即打断当前行动
