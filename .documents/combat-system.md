...
- `AttackBase` 的执行过程：
  ...
  - 根据反应类型执行不同逻辑：
    ...
    - 闪避：消耗 1 点反应并额外扣除 3 点行动点，攻击落空，受击方打断当前行动并位移
    - 承受：正常结算伤害
...
- 当前实现的攻击类型：
  ...
  - `HalfSwordAttack`（Thrust 半剑式）：只允许有武器的手臂使用，且另一只手必须有护甲（用于握持剑刃）且没有武器；伤害为刺击的80%；前摇3、后摇4；闪避影响0.75、格挡影响0.30；`ArmorGapPower`为2.0，使护甲覆盖率被平方处理，大幅提高击中盔甲缝隙的概率；玩家默认拥有此技能，AI会考虑此攻击但NPC暂不拥有
  - `KickAttack`（Special 特殊）：只允许腿使用
  - `HeadbuttAttack`（Special 特殊）：只允许头使用；头槌攻击腿部时闪避影响 0.98、格挡影响 0.95，攻击手臂时闪避影响 0.95、格挡影响 0.92，攻击其他部位时闪避/格挡影响 0.35；头槌不会导致任何角色倒伏；AI 只会用头槌攻击头部
...
- `ParryAction`（招架）：仅腿部可用；倒伏或被束缚时不可用；前摇 1、后摇 3 行动点；摆出招架姿态，当防御者执行招架动作期间成功格挡攻击时，会获得 +1 反应点数；AI 使用招架动作的权重为 5，相比其他动作较低。
...
- 反应类型包括：
  - 格挡（Block）：消耗 1 点反应，选择身体部位或装备承受伤害，伤害会转移到格挡目标；若格挡目标是当前行动使用的身体部位或其装备，会打断自身行动；格挡失败时，根据伤害值/伤害前部位HP的概率打断当前行动
  - 闪避（Dodge）：消耗 1 点反应并额外扣除 3 点行动点，打断自身当前行动并躲开攻击，攻击完全落空
  - 承受（Endure）：不消耗反应，正常承受伤害；根据伤害值/伤害前部位HP的概率打断当前行动
- 闪避必定打断自身当前行动；格挡分两种情况：使用当前行动部位格挡会立即打断，格挡成功不会因伤害打断，格挡失败时按概率打断
- 伤害打断机制：
  - 格挡成功时不会触发伤害打断判定
  - 格挡失败或承受时，打断概率为伤害值/伤害前的部位HP
  - 打断判定仅针对身体部位（BodyPart），对装备造成的伤害不会触发打断
  - 打断会将角色的 `combatAction` 置空，并显示提示文本"{角色名}的行动被打断!"
  - `performHit` 返回对身体部位造成的实际伤害值，由外层根据伤害值和反应类型判断是否打断
  - 打断规则会在反应菜单的格挡和承受选项描述中明确展示
...
- 闪避表现：闪避成功时播放 `retroClick` 音效，并移动到闪避位置后返回
- 格挡部位匹配度系统：
  - 使用不同身体部位格挡时会根据部位匹配度对格挡成功率进行修正，修正系数范围为 0.05（几乎不可能）到 1.0（无惩罚）
  - 通过 `AttackBase.CalculateBlockChanceModifier(ICombatTarget)` 方法计算，仅当攻击目标为身体部位时生效
  - 修正因素包括：
    - 高度差惩罚：基于格挡部位与攻击目标部位的归一化高度差，使用指数衰减函数 `exp(-heightDiff * 3.0)`，高度差越大惩罚越重
    - 特殊规则：用头格挡任何部位都极难（头对头30%，其他5%）；用腿格挡头部攻击极难（8%）
    - 加成项：使用武器格挡 +15%；同类型部位格挡（臂对臂、腿对腿）+10%；用躯干格挡 +10%
  - 最终修正系数会乘以基础格挡成功率，在玩家格挡菜单中每个选项显示调整后的成功率
  - AI在计算格挡期望伤害时也会使用调整后的成功率
  - 格挡判定时使用调整后的成功率进行骰子检定
...
- `AttackBase.ArmorGapPower` 属性控制护甲覆盖率的幂次处理，默认为1.0（不处理），半剑式为2.0（平方）；护甲命中判定使用 `Math.Pow(Coverage, ArmorGapPower)`，power越高越容易击中盔甲缝隙
