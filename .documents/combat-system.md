...
- `ReleaseAction`（放手）：仅在左右手臂出现；当该手臂存在擒拿 Buff 或手上握有武器时可用；若两者皆无则在菜单中禁用，AI 也不会把该行动加入候选；若身上存在擒拿 Buff，会解除自身施加的束缚并提示被解放的目标数量；若仅握有武器，会在执行时移除手中符合武器槽 (`ItemFlagCode.Arm`) 的物品并提示“丢下了{部位}的{武器名}”。
