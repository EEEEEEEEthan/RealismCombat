...
- `ReleaseAction`（放手）：仅在左右手臂出现；当该手臂存在擒拿 Buff 或手上握有武器时可用；若身上存在擒拿 Buff，会解除自身施加的束缚并提示被解放的目标数量；若仅握有武器，会在前摇阶段立即移除手中符合武器槽 (`ItemFlagCode.Arm`) 的物品并提示“丢下了{部位}的{武器名}”，不再出现“准备丢下”提示；AI 仅在需要解除擒拿时考虑该行动，避免无故把武器丢弃。
