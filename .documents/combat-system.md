...
- `PlayerInput` 的决策步骤：
  - 先列出自身所有可用身体部位生成菜单，并展示实时生命值与当前 Buff，允许通过返回选项重新开始
...
- `PickWeaponAction`（捡）：当双臂中存在空的武器槽且战斗的 `droppedItems` 中有与该槽匹配的武器时可用；玩家会列出可拾取的武器并选择其一，AI 随机选择；执行时从 `droppedItems` 移除被捡起的武器，若手上已有物品会先放回 `droppedItems` 再装备新武器；前摇 2、后摇 1 行动点，执行不造成伤害，仅拾取搬运。
...
- `PlayerInput` 的反应决策：
  ...
  - 若没有可用格挡目标会提示并重新选择
- 玩家反应菜单标题格式为“应对{攻击者武器/部位}对{防守者部位}的{攻击名}”，例如“应对贵族兵长剑对Ethan头部的斩击”
...
- 将护甲的 `Coverage` 相加得到总覆盖率，截断到 1；按权重随机命中某件护甲
- 命中护甲：伤害与耐久结算到该护甲，并使用护甲的防护值；未命中护甲则直接命中部位，防护为 0
