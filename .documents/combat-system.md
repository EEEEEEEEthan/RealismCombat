...
- `CombatInput` 是输入基类，提供阵营、目标筛选与界面辅助方法
  - `GetAvailableTargets()` 返回角色所有可用的身体部位
  - `GetAttackTargets()` 返回角色所有可攻击目标，包括身体部位和手臂上持有的武器/装备
  - `GetBlockTargets()` 返回角色所有可用于格挡的目标，包括身体部位和手臂上持有的武器/装备
- `PlayerInput` 的决策步骤：
  - 先弹出仅包含"攻击"的 `MenuDialogue`，为扩展其他指令保留入口
  - 取得敌方存活单位生成菜单，并允许通过返回选项回到上一层
  - 选择敌人后，调用 `GetAttackTargets()` 列出该敌人所有可攻击目标（包括身体部位和手上的武器），并展示实时生命值
  - 返回 `Attack` 行动实例，战斗主循环会在 `StartTask()` 中立即扣除前摇成本
- `AIInput` 使用 `GetAttackTargets()` 随机挑选敌方角色与攻击目标（可能是身体部位或武器）：
  - 若无法找到存活敌人或可攻击目标会抛出异常，主循环会捕获并记录
  - 在格挡决策中优先选择武器进行格挡，其次选择身体部位
