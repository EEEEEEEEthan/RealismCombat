...
- `PlayerInput` 的决策步骤：
  ...
  - 选择自身身体部位后，根据该部位可用的攻击类型生成菜单（如斩击、刺击、踢、头槌、撞击、抓取等），每个攻击选项的描述会显示：对于攻击类行动，首行为伤害预览（格式为`伤害 砍X 刺Y 钝Z`），第二行为行动力消耗（格式为`行动力 前摇X 后摇Y`），第三行为行动描述；对于非攻击类行动，首行为行动力消耗，第二行为行动描述
...
- `GenericAIInput` 的决策步骤：
  - 在决策开始时优先检查角色是否处于倒伏状态（任意身体部位有倒伏 Buff），若是则为所有可用手臂创建爬起动作，并赋予最高权重（100），确保倒伏角色优先尝试爬起
  - 为所有可用身体部位生成所有可用攻击类型的候选动作（斩击、刺击、踢、头槌、撞击、抓取等）
  - 为每个候选动作计算期望伤害权重：`期望伤害 × (1 - 最高反应成功率)`，作为该动作的选择权重
  - 添加特殊行动：若双臂无武器且有腰带武器，为拿武器动作赋予 50 权重；若手臂被束缚或擒拿，为放手动作赋予 50 权重
  - 从权重字典中选取权重最高的动作（权重相同时随机），尝试为该动作分配随机目标；若分配成功则返回该动作，否则移除该动作并重试下一个最高权重动作
  - 若所有候选动作都无法执行（例如倒伏时没有可用手臂），会抛出"AI无法执行任何行动"异常，主循环会捕获并记录
...
- `AttackBase` 的执行过程：
  ...
  - 根据反应类型执行不同逻辑：
    ...
    - 承受：正常结算伤害
- 通过 `Damage` 计算伤害值（通常为 1~3 点）写入最终目标的生命值，界面通过 `CharacterNode.Shake()` 与受击音效反馈
- 注意：`GrabAttack`（抓取攻击）不造成伤害，直接覆写 `Damage` 返回 0
  - 持续追加部位状态、倒地提示，并在对话框关闭后额外扣除 5 点行动点
- `AttackBase.Damage`：挥砍/刺击会遍历攻击部位槽位取首个 `ItemFlagCode.Arm` 武器的 `DamageProfile.Get(AttackType)`；特殊攻击需子类自行覆写 `Damage` 给出数值（默认 0，例如头槌返回 `Damage(0,0,1)`）
...
- 当前实现的攻击类型：
  ...
  - `HeadbuttAttack`（Special 特殊）：只允许头使用；若目标是手臂或躯干，有概率令目标获得[倒伏] Buff，概率为目标总重量/发起方总重量；若目标是腿部，发起方必定获得[倒伏] Buff；AI 只会用头槌攻击头部
  - `ChargeAttack`（Special 特殊）：只允许躯干使用，目标不能选择头颈；若目标为腿部，无论是否命中均会给发起者施加[倒伏] Buff；若攻击命中目标，有概率给目标施加[倒伏] Buff，概率为目标总重量/攻击者总重量
...
- 当部位存在束缚 Buff 时，除抽出外的行动都会在菜单中灰化；AI 也会跳过被灰化的行动
...
- `ReleaseAction`（放手）：仅在左右手臂出现；当该手臂存在擒拿 Buff 或手上握有武器时可用；若两者皆无则在菜单中禁用，AI 也不会把该行动加入候选；若身上存在擒拿 Buff，会解除自身施加的束缚并提示被解放的目标数量；若仅握有武器，会在执行时移除手中符合武器槽 (`ItemFlagCode.Arm`) 的物品并提示“丢下了{部位}的{武器名}”；行动描述会根据状态动态展示：有擒拿 Buff 时为“移除施加的束缚效果”，手持武器时为“扔掉手里的武器”。
- 当部位存在擒拿 Buff 时，除放手外的行动都会在菜单中灰化；AI 也会跳过被灰化的行动
...
- `GetUpAction`（爬起）：仅手部可用；当角色任意身体部位有倒伏 Buff 时显示；前摇 5、后摇 5 行动点；执行时解除角色所有身体部位的倒伏 Buff。
...
- 反应成功率计算：
  - 基础成功率受武器长度、重量、负重、攻击类型（闪避/格挡倾向）等因素影响
  - 若防守方任意部位有倒伏 Buff，闪避成功率为 0
  - 若防守方腿部被束缚（`!Free`），闪避成功率除以 5
  - 若防守方任意部位被束缚（`!Free`），闪避与格挡成功率均除以 3
- 反应类型包括：
  - 格挡（Block）：消耗 1 点反应，选择身体部位或装备承受伤害，伤害会转移到格挡目标；若格挡目标是当前行动使用的身体部位或其装备，会打断自身行动
  - 闪避（Dodge）：消耗 1 点反应，打断自身当前行动并躲开攻击，攻击完全落空
  - 承受（Endure）：不消耗反应，正常承受伤害
- 闪避必定打断自身当前行动；格挡仅在使用当前行动部位或其装备时打断行动
- `CombatAction.WillBeInterruptedByBlockingWith(ICombatTarget)` 方法用于判断用指定目标格挡是否会打断该行动，通过检查格挡目标是否是 `actorObject` 或其装备槽中的物品
- `PlayerInput` 的反应决策：
  - 弹出菜单让玩家选择反应类型
  - 选择格挡时会列出所有可用的格挡目标（可用身体部位与装备），标题为"用指定部位进行格挡"
  - 若没有可用格挡目标会提示并重新选择
  - 格挡选项描述会提示"使用当前行动部位格挡会打断自身行动"（当角色有进行中的动作时）
- 玩家反应菜单标题格式为"应对{攻击者武器/部位}对{防守者部位}的{攻击名}"，例如"应对贵族兵长剑对Ethan头部的斩击"；防守方部位标题仅显示裸部位名，不再拼接装备标签
- 反应菜单中文案：格挡选项在选择反应类型时不再显示成功率，在选择具体格挡部位时每个选项都会显示"成功率XX%"对应本次格挡判定成功率；闪避选项展示"成功率XX%"对应本次闪避判定成功率
- `GenericAIInput` 的反应决策：
  - 计算所有可用格挡方式的期望伤害，以 `1 / expectedDamage^3` 为权重随机抽取
  - 避免用腿格挡头部攻击（过滤掉此类选项）
  - 若格挡会打断自身行动，期望伤害乘以 1.5 作为惩罚，降低该选项的选择权重
  - 若无可用格挡目标则选择承受
- `GetBlockTargets()` 方法会返回角色的所有可用身体部位，以及双臂装备的可用装备
- 被束缚的身体部位不能用于格挡；被束缚部位物品槽（递归）里的 Item 也不能用于格挡；`GetBlockTargets()` 使用 `Free` 属性过滤不可用的格挡目标
...
- 闪避与格挡成功率：
  ...
  - 躯干与裆部格挡判定额外+0.35加成
  - 被束缚的是腿时，闪避概率除以 5（在 `AttackBase.ReactionChance` 中通过检查腿部 `Free` 属性实现）
- 各攻击的 `DodgeImpact`/`BlockImpact` 会根据攻击部位与目标部位的归一化高度差调整：`BodyPartCode.NormalizedHeight` 取值头1.0、躯干0.85、裆0.7、手臂0.75、腿0.35，差值≥0.4时大幅提高被闪避/格挡概率，用于表现头槌打脚、脚踢头等极端角度更易被防御
  - 武器攻击会以 `abs(高度差 - 武器长度)` 作为有效高度差参与上述判定，长武器在跨高度攻击时更容易被闪避或格挡
...
- 伤害抵御改为直接用 `Damage - Protection` 逐项相减并截断为 0，已移除 `ApplyProtection()` 的辅助方法
...
- `availableCombatActions` 保存角色可用行动列表，默认包含 `BreakFree` 以确保被束缚时总能使用抽出
